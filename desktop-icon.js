...
function initializeDesktopIcons() {
    ...
    interact('.desktop-icon-container')
        .draggable({
            listeners: {
                start(event) {
                    ...
                    if (target.classList.contains('selected')) {
                        const selectedIcons = document.querySelectorAll('.desktop-icon-container.selected');
                ...
                move(event) {
                    ...
                    if (target.classList.contains('selected')) {
                        const selectedIcons = document.querySelectorAll('.desktop-icon-container.selected');
                        selectedIcons.forEach(icon => {
                            if (icon !== target) { // Ne déplacez pas l'icône que vous déplacez actuellement à nouveau
                    ...
                    if (currentX === iconInitialPosition.x && currentY === iconInitialPosition.y) {
...
function handleIconSelection(iconContainer) {
    iconContainer.addEventListener('mousedown', function(event) {
        ...
        if (event.button === 0 && !iconContainer.classList.contains('selected') && selectedIcons.length > 0) {
        ...
        if (event.button === 0 && selectedIcons.length === 1 && !iconContainer.classList.contains('selected')) {
        ...
        const otherSelectedIcons = [...document.querySelectorAll('.desktop-icon-container.selected')].filter(ic => ic !== iconContainer);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener('mousedown', function (event) {
        if (!event.target.closest('.desktop-icon-container') && !event.target.closest('#context-menu')) {
            desktopIconContainers.forEach(iconContainer => {
                iconContainer.classList.remove('selected');
            });
        }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener('click', function (event) {
       ...
       if (!event.target.closest('.desktop-icon-container')) {
            desktopIconContainers.forEach(iconContainer => iconContainer.classList.remove('selected'));
        }
    });
...
function resetDesktopIcons() {
    const placeholders = document.querySelectorAll('.desktop-icon-container.placeholder');
...
document.addEventListener('click', function (event) {
    setTimeout(() => {
        const selectedIcons = document.querySelectorAll('.desktop-icon-container.selected');
        console.log(`Nombre d'icônes sélectionnées après clic : ${selectedIcons.length}`);
